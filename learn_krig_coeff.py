...
from models import PermPreserveClass, MyMaternKernel, MyNSKernel_Scale, MyNSKernel_Lengthscale
from input_transform import input_transformed_dim, input_transform

torch.manual_seed(1)
# %% tuning parameters
d = 2 # locs are sampled from R^d
target = 'krig_coeff' # ['y', 'cond_mean', 'cond_sd', 'krig_coeff']
input_trans_type = 'locs_lastloc' # ["locs", "locs_and_y", "locs_diff", "locs_diff_and_y", "dist_direction", "dist_direction_and_y", "dist_direction_lastloc", "dist_direction_lastloc_and_y", "locs_lastloc", "locs_lastloc_and_y"]
m = 30
...
dataloader = Vecc_Dataloader_GP_sim(KernelClass, kernel_parms_init, d, target)
# %% initialize model
model = PermPreserveClass(size_phi, size_rho1, size_rho2)
model.to(device)
# %% scheduler
def lr_lambda(epoch):
    base_lr = 0.001
...
for epoch in range(n_epoch): 
    with torch.no_grad():
        locs_batch, y_batch, y, length = dataloader.get_minibatch(size=n_batch, m=m)
...
# model_fn = f"krig_coeff_d{d}_{input_trans_type}_{kernel_gen_name}_loss{loss.detach().item():.4f}"
