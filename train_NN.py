...
for epoch in range(n_epoch): 
    with torch.no_grad():
        ...
        input_mean, input_sd, y_true, length = input_mean.to(device), \
            input_sd.to(device), y_true.to(device), length.to(device)
...
with torch.no_grad():
    ...
    y_pred_test_GP, y_stderr_test_GP = mdl_GP(X_batch, y_batch, length=length)
    loss_test_GP = loss_function(y_pred_test_GP, y_true, y_stderr_test_GP)
    print(f"NLL loss of GP is {loss_test_GP.detach().item()}", flush=True)
