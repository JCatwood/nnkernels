...
for epoch in range(n_epoch): 
    with torch.no_grad():
        ...
        X_batch, y_batch, y_true, length = X_batch.to(device), y_batch.to(device), y_true.to(device), length.to(device)
    # predict the target
    optimizer.zero_grad()
...
with torch.no_grad():
    ...
    y_pred, y_stderr = model(X_batch, y_batch, length=length)
    loss = loss_function(y_pred, y_true, y_stderr)
    print(f"NLL loss of the GP model is {loss.detach().item()}", flush=True)
    print(f"MSE loss of the GP model is {loss_MSE(y_pred, y_true).item()}", flush=True)
